#include <mowerbot/ahrs_driver.h>
#include <Eigen/Eigen>
namespace FDILink
{
ahrsBringup::ahrsBringup() :frist_sn_(false), serial_timeout_(20)
{
  ...
  pravite_nh.param("gnss_dual_ant_topic_", gnss_dual_ant_topic_, std::string("/gnss_dual_ant/fix"));
  ...
  gnss_dual_ant_pub_ = nh_.advertise<gps_common::GPSFix>(gnss_dual_ant_topic_.c_str(), 10);
...
void ahrsBringup::processLoop()   // 数据处理过程
{
  ...
  while (ros::ok())
  {
    ...
    if (head_type[0] != TYPE_IMU && head_type[0] != TYPE_AHRS && head_type[0] != TYPE_INSGPS && head_type[0] != TYPE_GEODETIC_POS
      && head_type[0] != TYPE_GNSS_DUAL_ANT && head_type[0] != 0x50 && head_type[0] != TYPE_GROUND && head_type[0] != 0xff)
    {
    ...
    else if (head_type[0] == TYPE_GNSS_DUAL_ANT && check_len[0] != GNSS_DUAL_ANT_LEN)
    {
      ROS_WARN("head_len error (GNSS_DUAL_ANT)");
      continue;
    }
    ...
    else if (head_type[0] == TYPE_GNSS_DUAL_ANT) // GNSS Dual Ant
    {
      GNSS_Dual_Ant_frame_.frame.header.header_start   = check_head[0];
      GNSS_Dual_Ant_frame_.frame.header.data_type      = head_type[0];
      GNSS_Dual_Ant_frame_.frame.header.data_size      = check_len[0];
      GNSS_Dual_Ant_frame_.frame.header.serial_num     = check_sn[0];
      GNSS_Dual_Ant_frame_.frame.header.header_crc8    = head_crc8[0];
      GNSS_Dual_Ant_frame_.frame.header.header_crc16_h = head_crc16_H[0];
      GNSS_Dual_Ant_frame_.frame.header.header_crc16_l = head_crc16_L[0];
      uint8_t CRC8 = CRC8_Table(GNSS_Dual_Ant_frame_.read_buf.frame_header, 4);
      if (CRC8 != GNSS_Dual_Ant_frame_.frame.header.header_crc8)
      {
        ROS_WARN("header_crc8 error");
        continue;
      }
      if(!frist_sn_){
        read_sn_  = GNSS_Dual_Ant_frame_.frame.header.serial_num - 1;
        frist_sn_ = true;
      }

      ahrsBringup::checkSN(TYPE_GNSS_DUAL_ANT);
    }
    ...
    {
      uint16_t head_crc16_l = GNSS_Dual_Ant_frame_.frame.header.header_crc16_l;
      uint16_t head_crc16_h = GNSS_Dual_Ant_frame_.frame.header.header_crc16_h;
      uint16_t head_crc16 = head_crc16_l + (head_crc16_h << 8);
      size_t data_s = serial_.read(GNSS_Dual_Ant_frame_.read_buf.read_msg, (GNSS_DUAL_ANT_LEN + 1)); //134+1

      uint16_t CRC16 = CRC16_Table(GNSS_Dual_Ant_frame_.frame.data.data_buff, GNSS_DUAL_ANT_LEN);
      if (if_debug_){
        std::cout << "CRC16:        " << std::hex << (int)CRC16 << std::dec << std::endl;
        std::cout << "head_crc16:   " << std::hex << (int)head_crc16 << std::dec << std::endl;
        std::cout << "head_crc16_h: " << std::hex << (int)head_crc16_h << std::dec << std::endl;
      ...
      else if(GNSS_Dual_Ant_frame_.frame.frame_end != FRAME_END)
      {
        ROS_WARN("check frame end.");
        continue;
      }
    ...
    // GNSS Dual Ant: 读取 gnss_dual_ant 数据进行解析，并发布相关话题
    else if (head_type[0] == TYPE_GNSS_DUAL_ANT)
    {
      ...
      //std::cout << "lat: " << GNSS_Dual_Ant_frame_.frame.data.data_pack.MBLat << std::endl;
      //std::cout << "lon: " << GNSS_Dual_Ant_frame_.frame.data.data_pack.MBLon << std::endl;
      //std::cout << "alt: " << GNSS_Dual_Ant_frame_.frame.data.data_pack.MBAlt << std::endl;

      gnss_dual_ant_pub_.publish(gnss_dual_ant_data);
    }
...
void ahrsBringup::checkSN(int type)
{
  ...
  case TYPE_GNSS_DUAL_ANT: // GNSS Dual Ant
    if (++read_sn_ != GNSS_Dual_Ant_frame_.frame.header.serial_num)
    {
      if ( GNSS_Dual_Ant_frame_.frame.header.serial_num < read_sn_)
      {
        sn_lost_ += 256 - (int)(read_sn_ - GNSS_Dual_Ant_frame_.frame.header.serial_num);
        if(if_debug_){
          ROS_WARN("detected sn lost.");
        }
      }
      ...
      {
        sn_lost_ += (int)(GNSS_Dual_Ant_frame_.frame.header.serial_num - read_sn_);
        if(if_debug_){
          ROS_WARN("detected sn lost.");
        }
      }
    }
    read_sn_ = GNSS_Dual_Ant_frame_.frame.header.serial_num;
    break;
